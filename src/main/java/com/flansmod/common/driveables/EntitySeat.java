...
public class EntitySeat extends Entity implements IControllable, IEntityAdditionalSpawnData
{
	...
	public void onUpdate()
	{
	    ...
	    if ((this.worldObj.isRemote) && 
	    	    ...
	    	    {
	    	      VehicleOverlay.setSpeed(this.driveable.throttle);
	    	      VehicleOverlay.setFuel(Math.round(this.driveable.driveableData.fuelInTank));
	    	    }
	...
	public boolean interactFirst(EntityPlayer entityplayer) //interact : change back when Forge updates
    {
        ...
        if(this.driveable instanceof EntityVehicle)
        {
        EntityVehicle v = (EntityVehicle)driveable;
		System.out.println(v.isVehicleLocked());
        if(v.isVehicleLocked())
        {
        	entityplayer.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.GOLD + "[" + EnumChatFormatting.RED + "CraftingLife" + EnumChatFormatting.GOLD + "]" + EnumChatFormatting.RED + " Le véhicule est verrouillé."));
             return false;
             
        }
		if(v.getVehicleType().shortName.equals(v.police)){
			System.out.println("C'est un vehicule de police !" + v.owner.getName());
		}
		else{
    ...
    {
        VehicleOverlay.setFuelMax(this.driveable.getDriveableData().fuelInTank);
        VehicleOverlay.setmaxSpeed(this.driveable.getDriveableType().maxThrottle);
        VehicleOverlay.setVehicle(this.driveable);
    }
