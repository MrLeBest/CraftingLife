	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer)
    {
        ...
        if(movingobjectposition.typeOfHit == MovingObjectType.BLOCK)
        {
            ...
            if(type.placeableOnLand || block instanceof BlockLiquid)
            {
	            if(!world.isRemote)
	            {
	            	EntityVehicle vehicle = new EntityVehicle(world, (double)i + 0.5F, (double)j + 2.5F, (double)k + 0.5F, entityplayer, type, getData(itemstack, world));
	            	vehicle.setOwner(entityplayer.getGameProfile());
