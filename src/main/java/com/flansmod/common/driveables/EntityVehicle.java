...
import com.flansmod.client.MovingSoundDriveable;
...
public class EntityVehicle extends EntityDriveable implements IExplodeable
{
	...
	public void readSpawnData(ByteBuf data)
	{
        ...
        this.owner = NBTUtil.func_152459_a(tag);
	}
    ...
    protected void writeEntityToNBT(NBTTagCompound tag)
    {
        ...
        NBTUtil.func_152460_a(tag, this.owner);
        tag.setBoolean("vehiclelock", this.isVehicleLocked());
    }
    ...
    protected void readEntityFromNBT(NBTTagCompound tag)
    {
        ...
        this.owner = NBTUtil.func_152459_a(tag);
        this.setVehicleLocked(tag.getBoolean("vehiclelock"));
    }
    ...
    {
		System.out.println(this.isVehicleLocked());
		if(!entityplayer.getGameProfile().equals(this.owner) && this.isVehicleLocked())
		{
			
		    return false;
		}
		
		if(this.getVehicleType().shortName.equals(this.police)){
			System.out.println("C'est un vehicule de police !" + owner.getName());
		}
		else{
			System.out.println("EHhh non");
		}
	...
	public boolean pressKey(int key, EntityPlayer player)
	{
		...
		switch(key)
		{
            ...
            case 21 :
            {
                this.worldObj.playSound(this.posX, this.posY, this.posZ, "craftingmod:klaxon", 1.0F, 1.0F, false);
            }
            ...
            case 22:
            {
                ...
                if(isSirenActive()) {
                    player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.GOLD + "[" + EnumChatFormatting.GREEN + "CraftingLife" + EnumChatFormatting.GOLD + "]" + EnumChatFormatting.GREEN + " Sirene de police activé !"));
                    playSirenSound(this);

                }else {
                    player.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.GOLD + "[" + EnumChatFormatting.RED + "CraftingLife" + EnumChatFormatting.GOLD + "]" + EnumChatFormatting.RED + " Sirene de police désactivé !"));
                }
    ...
    public void setVehicleLocked(boolean b)
    {
        this.dataWatcher.updateObject(2, (b ? (byte)1 : (byte)0));
    }
    ...
    public void setSirenActive(boolean b)
    {
      this.dataWatcher.updateObject(3, Byte.valueOf((byte)(b ? 1 : 0)));
    }
    
    @SideOnly(Side.CLIENT)
    public static void playSirenSound(EntityVehicle vehicle)
    {
      Minecraft.getMinecraft().getSoundHandler().playSound(new MovingSoundDriveable(new ResourceLocation("craftingmod", "policesirene"), vehicle));
