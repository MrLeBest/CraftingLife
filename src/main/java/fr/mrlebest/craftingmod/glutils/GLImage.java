package fr.mrlebest.craftingmod.glutils;
...
public class GLImage
{
    public int        h           = 0;
    public int        w           = 0;
    public ByteBuffer pixelBuffer = null; // store pixels as bytes in GL_RGBA
                                          // format
    public int[]      pixels      = null; // store pixels as ARGB integers

    // Used by GLUtils.drawImage() to hold image as a texture
    // The texture will be allocated only if needed (see GLUtils.drawImage()
    public int        textureHandle;     // this image stored as a texture
    public int        textureW;          // the power-of-two dimensions that
                                          // can hold this image
                                          // (ie. an image 250x200 would have
                                          // textureSize 256)
    public int        textureH;
    ...
    public GLImage(String imgName)
    {
        ...
        {
            GLUtils.msg("GLImage(String): loaded " + imgName + ", width=" + w
                    + " height=" + h);
        }
    }
    ...
    public GLImage(String imgName, boolean flipYaxis, boolean convertPow2)
    {
        ...
        {
            GLUtils.msg("GLImage(String,bool,bool): loaded " + imgName
                    + ", width=" + w + " height=" + h);
        }
    }

    /**
     * Create GLImage from image file bytes (the contents of a jpg, gif or png
     * file). Flip Y axis.
     * 
     * @param pixels
     * @param w
    ...
    public GLImage(byte[] bytes)
    {
        ...
        if(makeGLImage(img, true, false))
        {
            GLUtils.msg("GLImage(byte[]): loaded image from bytes["
                    + bytes.length + "]");
        }
        else
        {
            GLUtils.err("GLImage(byte[]): could not create Image from bytes["
                    + bytes.length + "]");
        }
    }
    ...
    public GLImage(byte[] bytes, boolean flipYaxis, boolean convertPow2)
    {
        ...
        if(makeGLImage(img, flipYaxis, convertPow2))
        {
            GLUtils.msg("GLImage(byte[],bool,bool): loaded image from bytes["
                    + bytes.length + "]");
        }
        else
        {
            GLUtils.err("GLImage(byte[],bool,bool): could not create Image from bytes["
                    + bytes.length + "]");
        }
    }
    ...
    public BufferedImage makeBufferedImage(byte[] imageFileContents)
    {
        ...
        {
            GLUtils.err("GLImage.makeBufferedImage(): " + ioe);
        }
        return bi;
    }

    /**
     * Create GLImage from pixels passed in a ByteBuffer. This is a non-standard
     * approach that may give unpredictable results.
     * 
     * @param pixels
     * @param w
     * @param h
     ...
     * @param imgName
     */
    public boolean makeGLImage(BufferedImage tmpi, boolean flipYaxis,
            boolean convertToPow2)
    {
        if(tmpi != null)
        {
            if(flipYaxis)
            ...
            pixelBuffer = convertImagePixelsRGBA(pixels, w, h, false); // convert
                                                                       // to
                                                                       // bytes
                                                                       // in
                                                                       // RGBA
                                                                       // format
            textureW = GLUtils.getPowerOfTwoBiggerThan(w); // the texture size
                                                           // big enough to
                                                           // hold this image
            textureH = GLUtils.getPowerOfTwoBiggerThan(h); // the texture size
                                                           // big enough to
                                                           // hold this image
            // GLUtils.msg("GLImage: loaded " + imgName + ", width=" + w +
            // " height=" + h);
            return true;
        }
        else
        {
            // GLUtils.err("GLImage: FAILED TO LOAD IMAGE " + imgName);
            pixels = null;
            pixelBuffer = null;
            h = w = 0;
            return false;
    ...
    public BufferedImage loadJavaImage(String imgName)
    {
        ...
        if(imgName.toLowerCase().endsWith(".tga"))
        {
            try
            {
                InputStream in = GLUtils.getInputStream(imgName);
                if(in != null)
                {
                    ...
                    out.flush();
                    out.close();
                    tmpi = decodeTGA(out.toByteArray());
                }
                else
                {
                    GLUtils.err("GLImage.loadJavaImage() exception: COULDN'T FIND IMAGE : "
                            + imgName);
                }
            }
            catch (IOException e)
            {
                GLUtils.err("GLImage.loadJavaImage() exception: FAILED TO LOAD IMAGE : "
                        + imgName + " :" + e);
                e.printStackTrace();
            }
        }
            ...
            try
            {
                tmpi = ImageIO.read(GLUtils.getInputStream(imgName));
            }
            catch (IOException e)
            {
                GLUtils.err("GLImage.loadJavaImage() exception: FAILED TO LOAD IMAGE : "
    ...
    public static int[] getImagePixels(Image image)
    {
        ...
        if(image != null)
        {
            ...
            PixelGrabber pg = new PixelGrabber(image, 0, 0, imgw, imgh,
                    pixelsARGB, 0, imgw);
            try
            {
                pg.grabPixels();
    ...
    /**
     * Copy ARGB pixels to a ByteBuffer without changing the ARGB byte order. If
     * used to make a texture, the pixel format is GL12.GL_BGRA. With this
     * format we can leave pixels in ARGB order (faster), but unfortunately I
     * had problems building mipmaps in BGRA format (GLU.gluBuild2DMipmaps() did
     * not recognize GL_UNSIGNED_INT_8_8_8_8 and GL_UNSIGNED_INT_8_8_8_8_REV
     * types so screwed up the BGRA/ARGB byte order on Mac).
     * 
     * @return ByteBuffer
     */
    public static ByteBuffer convertImagePixelsARGB(int[] jpixels, int imgw,
            int imgh, boolean flipVertically)
    {
        ...
        ByteBuffer bb = GLUtils.allocBytes(jpixels.length * 4); // 4 bytes per
                                                                // pixel
     ...
     * @return ByteBuffer
     */
    public static ByteBuffer convertImagePixelsRGBA(int[] jpixels, int imgw,
            int imgh, boolean flipVertically)
    {
    ...
    /**
     * Same function as in GLUtils.java. Allocates a ByteBuffer to hold the
    ...
    public void convertToPowerOf2()
    {
        ...
        pixels = getImagePixels(scaledImg); // pixels in default Java ARGB
                                            // format
        pixelBuffer = convertImagePixelsRGBA(pixels, w, h, false); // convert to
                                                                   // bytes in
                                                                   // RGBA
                                                                   // format
        textureW = GLUtils.getPowerOfTwoBiggerThan(w); // the texture size big
                                                       // enough to hold this
                                                       // image
        textureH = GLUtils.getPowerOfTwoBiggerThan(h); // the texture size big
                                                       // enough to hold this
                                                       // image
    }
     ...
     */
    public static void savePixelsToPNG(int[] pixels, int width, int height,
            String imageFilename, boolean flipY)
    {
        if(pixels != null && imageFilename != null)
        {
            if(flipY)
            {
                // flip the pixels vertically (opengl has 0,0 at lower left,
                // java is upper left)
            ...
            try
            {
                ...
                javax.imageio.ImageIO.write(image, "png", new File(
                        imageFilename));
            }
            catch (Exception e)
            {
                GLUtils.err("GLImage.savePixelsToPNG(" + imageFilename
                        + "): exception " + e);
            }
        }
    }
    ...
    public static BufferedImage convertToPowerOf2(BufferedImage bsrc)
    {
        ...
        int newW = GLUtils.getPowerOfTwoBiggerThan(bsrc.getWidth());
        int newH = GLUtils.getPowerOfTwoBiggerThan(bsrc.getHeight());
        ...
        {
            AffineTransform at = AffineTransform.getScaleInstance((double) newW
                    / bsrc.getWidth(), (double) newH / bsrc.getHeight());
