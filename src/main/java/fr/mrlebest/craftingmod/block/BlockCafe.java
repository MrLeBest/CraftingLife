...
public class BlockCafe extends Block
{

    public BlockCafe(Material material)
    {
        super(material);
    }
    ...
    {
        this.setBlockBounds(0.0F, 0.0F, 0.0F, 1.0F, 2.2F, 1.0F);
    }
    ...
    public boolean rotateBlock(World world, int x, int y, int z, ForgeDirection axis)
    {
        if((axis == ForgeDirection.UP || axis == ForgeDirection.DOWN) && !world.isRemote && world.getBlockMetadata(x, y, z) == 0)
        {
            ...
            if(tile instanceof TileEntityCafe)
            {
            	TileEntityCafe tileDirectional = (TileEntityCafe)tile;
