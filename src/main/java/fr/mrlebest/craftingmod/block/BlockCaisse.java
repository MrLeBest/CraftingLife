...
public class BlockCaisse extends Block
{
    ...
    public BlockCaisse(Material material)
    {
        super(material);
    }
       ...
       if(stack.getItemDamage() == 0)
        {
           ...
           if(tile instanceof TileEntityCaisse)
            {
               int direction = MathHelper.floor_double((double)(living.rotationYaw * 4.0F / 360.0F) + 2.5D) & 3;
                ((TileEntityCaisse)tile).setDirection((byte)direction);
            }
       }
    }
        ...
        if((axis == ForgeDirection.UP || axis == ForgeDirection.DOWN) && !world.isRemote && world.getBlockMetadata(x, y, z) == 0)
             {
                 ...
                 if(tile instanceof TileEntityCaisse)
                 {
                	 TileEntityCaisse tileCasse = (TileEntityCaisse)tile;

                byte direction = tileCasse.getDirection();
               direction++;
                if(direction > 36)
                     {
                ...
                tileCasse.setDirection(direction);
                     return true;
                 }
             }
             return false;
