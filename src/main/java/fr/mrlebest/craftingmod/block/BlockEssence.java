...
public class BlockEssence extends Block
{
    ...
    public BlockEssence(Material material)
    {
        super(material);
    }
    ...
    public boolean rotateBlock(World world, int x, int y, int z, ForgeDirection axis)
    {
        if((axis == ForgeDirection.UP || axis == ForgeDirection.DOWN) && !world.isRemote && world.getBlockMetadata(x, y, z) == 0)
        {
            ...
            if(tile instanceof TileEntityEssence)
            {
            	TileEntityEssence tileDirectional = (TileEntityEssence)tile;
