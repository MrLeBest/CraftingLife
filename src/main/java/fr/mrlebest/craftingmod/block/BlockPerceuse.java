...
public class BlockPerceuse extends Block
{
    
    public BlockPerceuse(Material material)
    {
        super(material);
    }
    ...
    public boolean rotateBlock(World world, int x, int y, int z, ForgeDirection axis)
    {
        if((axis == ForgeDirection.UP || axis == ForgeDirection.DOWN) && !world.isRemote && world.getBlockMetadata(x, y, z) == 0)
        {
            ...
            if(tile instanceof TileEntityPerceuse)
            {
                TileEntityPerceuse tileDirectional = (TileEntityPerceuse)tile;
